function LEM_data_viewer_tom3108 (type, save_data, view,idx)
...
if isequal(type,'typical')
    im = im_typical;
elseif isequal(type,'dyslexic')
    im = im_dyslexic;
elseif isequal(type,'typical_s')
    im = im_typical_s;    
end
...
% load data
dim_vec = 10; %dimension
k_vec = 10; %nearest neighbors
for dim = dim_vec;
    for k = k_vec
        if isequal(type,'typical')
        ...
        if view == 1
            ...
            for K = K_vec
                ...
                [idx,~] = kmeans(f,K,'distance','correlation'); %idx - clustering, C (2nd param)- centroid locations
                i = 1;
                for slice = slice_vec
            ...
            elseif isequal(type,'typical_s') && save_data
...
%                 data = LEM_mat;
                rng(1); %for reproducibility
%                 [idx,~] = kmeans(data,K); %idx - clustering, C (2nd param)- centroid locations
%                 idx=c;
                %view each cluster
                data_clustered = reshape(idx,size(original_data,1),size(original_data,2),size(original_data,3));
                for i = 1:K                

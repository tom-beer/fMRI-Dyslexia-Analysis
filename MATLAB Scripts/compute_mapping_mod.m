function [mappedA, mapping] = compute_mapping_mod(A, type, no_dims, varargin)
...
%     end
    ...
    switch type
        ...
        case {'LEM1'}
            % Compute Laplacian Eigenmaps-based mapping
            if isempty(varargin), [mappedA] = LEM1(A, no_dims, 12, 1, eig_impl);
			elseif length(varargin) == 1, [mappedA] = LEM1(A, no_dims, varargin{1}, 1, eig_impl); %mappedA is G
            elseif length(varargin) > 1,  [mappedA] = LEM1(A, no_dims, varargin{1}, varargin{2}, eig_impl); end
            %mapping.name = 'Laplacian1';
            mapping = [];
            
        case {'LEM2'}
            % Compute Laplacian Eigenmaps-based mapping
            if isempty(varargin), [mappedA, mapping] = LEM2(G, no_dims, 12, 1, eig_impl);
			elseif length(varargin) == 1, [mappedA, mapping] = LEM2(L,D, no_dims, varargin{1}, 1, eig_impl);
            elseif length(varargin) > 1,  [mappedA, mapping] = LEM2(L,D, no_dims, varargin{1}, varargin{2}, eig_impl); end
            mapping.name = 'Laplacian2';
            
        case 'LaplacianCorr'
            % Compute Laplacian Eigenmaps-based mapping
            if isempty(varargin), [mappedA, mapping] = laplacian_eigen_corr(A, no_dims, 12, 1, eig_impl);
			elseif length(varargin) == 1, [mappedA, mapping] = laplacian_eigen_corr(A, no_dims, varargin{1}, 1, eig_impl);
            elseif length(varargin) > 1,  [mappedA, mapping] = laplacian_eigen_corr(A, no_dims, varargin{1}, varargin{2}, eig_impl); end
            mapping.name = 'LaplacianCorr';
        ...
        case 'LLE'
            % Compute LLE mapping
			if isempty(varargin), [mappedA, mapping] = lle_mod(A, no_dims, 12, eig_impl);
            else [mappedA, mapping] = lle_mod(A, no_dims, varargin{1}, eig_impl); end
            mapping.name = 'LLE';

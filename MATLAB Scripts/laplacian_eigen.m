...
%         [mappedX, lambda] = eigs(M, no_dims + 1, tol, options);

        
%         M = full(M);
%         [mappedX, lambda,~] = svd(M);

%         [mappedX, lambda,~] = svds(M',no_dims + 1,0);
        lambdaMax = eigs(M,1);
        M_tmp = lambdaMax * speye(size(M)) - M;
        [mappedX, lambda,~] = pca2(M_tmp, no_dims + 1, 20, 10000*2);
